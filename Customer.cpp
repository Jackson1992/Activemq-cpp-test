...
class Consumer : public ExceptionListener,
 ...
 public:
  ...
  virtual void onMessage(const Message* message) {
    ...
    if (count == 0) gettimeofday(&start1, NULL);
    try {
      count++;
      if (count != 0) {
        timeval end;
        gettimeofday(&end, NULL);
        cout << "receive message time" << count << "from first message:"
             << (end.tv_usec - start1.tv_usec) / 1000.0 +
                    (end.tv_sec - start1.tv_sec) * 1000.0 << endl;
      }
